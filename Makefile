...
.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dp-search-reindex-batch
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dp-search-reindex-batch
...
debug: ## Used to run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dp-search-reindex-batch
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dp-search-reindex-batch
...
test-component: ## Runs component test suite
	go test -cover -coverpkg=github.com/ONSdigital/dp-search-reindex-batch/... -component
